...
const Footer = () => (
  <footer className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} mb-8 w-full flex-col md:flex-row`}>
      ...
      <div className="md:mt-o mt-10 flex w-full flex-[1.5] flex-row flex-wrap justify-between">
          ...
          >
            ...
            <ul className="mt-4 list-none">
              {footerLink.links.map((link, index) => (
                <li
                  key={link.name}
                  className={`cursor-pointer font-poppins text-[16px] font-normal leading-[24px] text-dimWhite hover:text-secondary ${index !== footerLink.links.lenght - 1 ? 'mb-4' : 'mb-0'}`}
                >
                  {link.name}
                </li>
              ))}
      ...
      <div className="mt-6 flex flex-row md:mt-0">
        {socialMedia.map((social, index) => (
          <img
            ...
            src={social.icon}
            alt={social.id}
            className={`h-[21px] w-[21px] cursor-pointer object-contain ${index !== socialMedia.lenght - 1 ? 'mr-6' : 'mr-0'}`}
          />
        ))}
      </div>
    </div>
